...
package gs {
	...
	public class TweenLite {
		public static var version:Number = 6.32;
		...
		public function TweenLite($target:Object, $duration:Number, $vars:Object) {
			...
			if (this.vars.visible != undefined) {
				this.vars.parsed_visible = Boolean(this.vars.visible);
				if (this.vars.runBackwards == true) {
					this.vars.parsed_visible = !(Boolean(this.vars.visible) && !this.target.visible);
				}
			}
			this.tweens = [];
			_subTweens = [];
		...
		public function initTweenVals($hrp:Boolean = false, $reservedProps:String = ""):void {
			...
			if (this.target is Array) {
				...
				for (p in this.vars) {
					if (p == "ease" || p == "delay" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "onCompleteScope" || p == "runBackwards" || p == "visible" || p == "parsed_visible" || p == "onUpdate" || p == "onUpdateParams" || p == "onUpdateScope" || p == "autoAlpha" || p == "onStart" || p == "onStartParams" || p == "onStartScope" ||p == "renderOnStart" || p == "proxiedEase" || p == "easeParams" || ($hrp && $reservedProps.indexOf(" " + p + " ") != -1)) { //"type" is for TweenFilterLite, and it's an issue when trying to tween filters on TextFields which do actually have a "type" property.
			...
			if (this.vars.parsed_visible == true) {
		...
		public function complete($skipRender:Boolean = false):void {
			...
			if (this.vars.parsed_visible != undefined) { 
				this.target.visible = this.vars.parsed_visible;
			}
